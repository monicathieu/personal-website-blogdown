...
```{bash}
ls -lh --color /Library/Frameworks/R.framework/Versions
```
You should see that the folder for your R install is still there!

You can _also_ see when you `ls -l` the content of `/Library/Frameworks/R.framework/Versions` that the "Current" folder, which is what RStudio calls by default, is merely symlinked to a specific R version, with multiple installed at the same time. If you change which R version "Current" points to, you could change which version RStudio runs with!
...
If you want to be hardcore and do it without installing any extra software, [the official Posit instructions](https://support.posit.co/hc/en-us/articles/200486138-Changing-R-versions-for-the-RStudio-Desktop-IDE) also tell you that you can manually symlink the "Current" folder to the R version you want to use. For example, this will set me up to use R 4.3:

```bash
ln -s /Library/Frameworks/R.framework/Versions/Current /Library/Frameworks/R.framework/4.3-arm64
```

However, the Posit instructions also mention that you can use RSwitch, so no need to be a hero 😜
